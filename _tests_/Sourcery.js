...
const Sourcify = (key, route, onUpdate) => {
  const {
    url,
    store,
    useStored,
  ...
  let requestFunction = makeUri((body, uri) => {
    ...
    if (bodyType && !body) throw new Error(`Missing body in request: ${uri}`);
...
const createApi = (requests, onUpdate) => {
  ...
  for (const key in requests) {
    if (requests.hasOwnProperty(key)) {
      ...
      req.config.method = typeof req.config.method === 'string' && req.config.method.toLowerCase() || 'get';
      const {
        method
      } = req.config;
      if (result[method] === undefined) result[method] = {};
      result[method][key] = Sourcify(key, req, typeof onUpdate === 'function' ? onUpdate(key) : null);
    }
  }
